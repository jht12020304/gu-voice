...
class PatientSettingsPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PatientSettingsPage> createState() =>
      _PatientSettingsPageState();
}
...
class _PatientSettingsPageState extends ConsumerState<PatientSettingsPage> {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      setState(() {
        _saveOk = true;
        _saveMsg = t('common.patient.settings.savedMessage');
      });
    } catch (_) {
      setState(() {
        _saveOk = false;
        _saveMsg = t('common.patient.settings.saveFailedMessage');
      });
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [_profileTab(), _notificationsTab(), _securityTab()],
        ),
      ),
    );
  }
  ...
  Widget _profileTab() {
    ...
    return ListView(
      ...
      children: [
        Text(
          t('common.patient.settings.profileTitle'),
        ...
        TextField(
          ...
          enabled: false,
          decoration: InputDecoration(
            labelText: t('common.patient.settings.emailLabel'),
          ),
        ),
        const SizedBox(height: 12),
        TextField(
          ...
          decoration: InputDecoration(
            labelText: t('common.patient.settings.phoneLabel'),
          ),
        ),
        const SizedBox(height: 16),
        Text(
          t('common.patient.settings.preferredLanguage'),
              ...
              ChoiceChip(
                ...
                label: Text(
                  t('common.language.names.$lng') +
                      (betaLanguages.contains(lng)
                          ? ' ${t('common.language.betaTag')}'
                          : ''),
                ),
                onSelected: (_) {
                  ...
                  context.go(
                    uri
                        .replace(path: prefixLngToPath(uri.path, lng))
                        .toString(),
                  );
                },
              ),
        ...
        Text(t('common.fontSize'), style: theme.textTheme.bodyMedium),
        Text(t('common.fontSizeHint'), style: theme.textTheme.bodySmall),
        const SizedBox(height: 8),
        SegmentedButton<double>(
          ...
          selected: {ref.watch(settingsProvider).textScale},
          onSelectionChanged: (v) =>
              ref.read(settingsProvider.notifier).setTextScale(v.first),
        ),
        const SizedBox(height: 20),
        FilledButton(
  ...
  Widget _notificationsTab() {
    return ListView(
      ...
      children: [
        Text(
          t('common.patient.settings.notificationsTitle'),
  ...
  Widget _securityTab() {
    return ListView(
      ...
      children: [
        Text(
          t('common.patient.settings.securityTitle'),
          style: Theme.of(context).textTheme.titleMedium,
        ),
        const SizedBox(height: 12),
        ListTile(
...
class _ChangePasswordDialogState extends State<_ChangePasswordDialog> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      setState(() {
        _busy = false;
        _error = t('common.patient.settings.passwordChangeFailed');
      });
    }
  }
      ...
      content: Column(
        ...
        children: [
          TextField(
            ...
            obscureText: true,
            decoration: InputDecoration(
              labelText: t('common.patient.settings.currentPassword'),
            ),
          ),
          const SizedBox(height: 8),
          TextField(
            ...
            obscureText: true,
            decoration: InputDecoration(
              labelText: t('common.patient.settings.newPassword'),
              helperText: t('common.patient.settings.passwordHint'),
            ),
          ),
          const SizedBox(height: 8),
          TextField(
            ...
            obscureText: true,
            decoration: InputDecoration(
              labelText: t('common.patient.settings.confirmPassword'),
            ),
          ),
          if (_error != null) ...[
            const SizedBox(height: 8),
      ...
      actions: [
        ...
        FilledButton(
          ...
          child: Text(
            _busy
                ? t('common.saving')
                : t('common.patient.settings.changePassword'),
          ),
        ),
      ],
    );
