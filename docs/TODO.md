...
>
> **2026-08-22 當天後續（皆已 merge＋後端已部署）**：
> - 真機首測抓到生產 OPENAI_API_KEY 是死 key（TTS/LLM 全 401）——已換、已驗（教訓：
>   密鑰類環境變數要有煙霧測試打真服務，healthz 看不出來）
> - 模型全面換 gpt-5.6 世代（PR #62；權威=config.py；取樣參數鐵律見
>   voice-pipeline-invariants #33）
> - 問診設計稽核 16 宣稱/15 成立/0 駁回（PR #63 修 SOAP 表單指令＋結束鈕離線守衛；
>   不變式 #34）。**稽核留下的 partial backlog**：感謝頁不顯示報告生成進度（12-poll
>   只在手動路徑的 SessionCompletePage）、supervisor 降級時每場跑滿硬上限、REST 終態
>   改不到病患畫面（跨 worker 無 bridge，文件已記為刻意）、紅旗語音警示 TTS 仍 TODO
> - **醫師＝管理員**＋醫師可代病患發起語音問診（PR #64；病患詳情頁麥克風鈕，場次記在
>   該病患名下）
> - 登入頁 taste-skill 重設計＋醫師/病患雙帶入鈕（PR #65；build 202608221816 已發
>   先行測試群組；憑證機制見 ios_release_settings.md §8.6）
...
| # | 需求 | 現行行為 |
|---|---|---|
| X-1 | 手機桌面推播、點開查看、已讀後紅色徽章消失 | ✅ **2026-09-09 真機複驗結案**：醫師手機在鎖定畫面收到推播。端到端實跑一場問診（指定醫師、完成送出）→ `session_complete` 與 `report_ready` 兩則推播任務皆 `sent:2 apns_sent:2 failed:0`，全走 Apple 原生 APNs 直送、一次都沒退到 FCM。⚠️ 先前「只有 App 內跳通知」的真因不是推播壞掉——正式環境從 09-03 07:38 到 09-09 之間根本沒有任何一場問診，站內看到的是 WebSocket 事件（與推播完全獨立的另一條路）|
...
- 可選醫師＝active、科別非空，且角色為 doctor，或角色為 admin 且有 `license_number`；
  ...
  **2026-09-09 補上專案負責人本人的臨床帳號後為 4 個**（同樣是 admin ＋ license「待補」，
  故為臨床 admin、只讀自己被指派的資料）。名單見 admin → 使用者管理，不抄進公開 repo。
