...
>
> 最後更新：2026-08-23（先行測試最新 build `202608231805`＝admin 刪除問診版）
...
目前群組狀態：**先行測試群組最新 = `202608231805`（4 位測試員都在這個群組）；
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202608231805`（版本 1.0.0，最新＝admin 刪除問診版） |
| 上傳時間 | 2026-08-21 20:46 | 2026-08-23 18:10 |
| 到期 | **2026-11-19** | **2026-11-21**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | `8f7a8632-2d07-44a5-87f4-f62a178b3fb0`（1805） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 1410 → 1728 → 1805 |
...
- **KIOSK 模式（2026-08-23 起，產品功能非測試輔助）**：`KIOSK_EMAIL` /
  ...
  （重開機即失；掉了用 admin 重設密碼再重打包）。
- 目前先行測試的 build `202608231805`（2026-08-23，admin 刪除問診版）：帶 KIOSK 兩參數、**不帶任何 E2E define**——帶入鈕與內嵌憑證皆為編譯期死碼（第 3 關印「測試期/kiosk dart-define：KIOSK_EMAIL KIOSK_PASSWORD」存證，清單裡沒有任何 E2E 項；純乾淨版 `202608231321` 當時印的是「無（正式版行為）」）。§8.6 的「最終版要拆」自此兌現；兩個測試帳號（測試員/測試醫師）仍在
  production 供 e2e 工具使用，**生產收真病歷前仍須停用測試醫師帳號**（撤銷條件不變）。
- ⚠️ **三位測試員帳號 2026-08-23 起是 `admin` 角色**（使用者指示：這三個帳號要有
  刪除問診的權限）。連帶效果：admin 讀得到全院病歷（本來 doctor 也是）、可用
  `DELETE /api/v1/sessions/{id}` 軟刪除任何一場問診、admin 四頁全開。
  推播 fan-out 同輪擴成 doctor + admin，否則升級後會**無聲**收不到任何通知。
...
- ⚠️ **醫師那組是「禁止內嵌 doctor 憑證」鐵律的使用者明示破例**（2026-08-22），
  僅限 TestFlight 內測建置；醫師帳號登入即可讀全部病歷（後端無 tenant 隔離）。
