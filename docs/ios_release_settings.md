...
`2026` < `202608221528（先行測試群組專用，內建自動登入）；全群組最新為 202608220710（前一顆 202608211213）`，CFBundleVersion 是逐段比較的，換過去會被判成未遞增而永久拒收。
...
## 8.6 測試期自動登入（2026-08-22，最終版要拆）

使用者拍板「登入最後才設定」。做法**不是**拿掉認證（後端每支 API 都要 JWT），
而是 build 時內建測試帳號自動登入：

- `AuthNotifier.bootstrap()`：冷啟動且無既存 session 時，若 build 帶了
  `--dart-define=E2E_EMAIL/E2E_PASSWORD` 就自動登入。**登出後不會搶著登回去**
  （否則永遠登不出、換不了帳號）；登入頁保留「測試：帶入帳密」一鍵鈕。
- 內建帳號是 **production 上的專用 patient 測試帳號**
  `hsuchun0304+guvoice-autotest@gmail.com`（名稱「測試員（勿用真實資料）」，
  無任何真實資料、只讀得到自己的場次）。**遵守既有鐵律：禁止內嵌 doctor/admin
  憑證**——那等於把全院病歷的鑰匙藏在安裝包裡。密碼只在本機與該 build 內。
- 要測醫師端：登出 → 手動輸入醫師帳密（自動登入不會攔截）。
- **最終正式版：打包時不帶這兩個 define，整段在編譯期就是死碼**，登入行為
  完全回復正常。這就是「最後才設定」的機制，不用改碼。

散佈：ASC 上新開內部群組「先行測試」（只有 `jht12020304@gmail.com` 一人，
用 API 建的），build `202608221528` 只發這個群組——原 4 人群組不會收到通知。
