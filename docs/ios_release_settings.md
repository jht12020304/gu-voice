...
- 真實測試員三帳號（2026-08-23 由使用者指示建立，密碼原要求 0000，後端政策
  ≥8 碼＋大小寫＋數字 → 統一 `Gu000000`；醫師必填 license_number 先以「待補」
  占位，請在 admin → 使用者管理補真值）：名單與 email 見 ASC/使用者管理，
  不抄進公開 repo。楊佳倫無「醫學生」角色可選 → 建為醫師、科別「泌尿科（醫學生）」。
