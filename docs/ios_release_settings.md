...
>
> 最後更新：2026-08-23（先行測試最新 build `202608231728`＝登入頁精簡版：無標誌、
> 無忘記密碼／註冊入口、語言下拉、kiosk 鈕 56pt）
...
目前群組狀態：**先行測試群組最新 = `202608231728`（4 位測試員都在這個群組）；
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202608231728`（版本 1.0.0，最新＝登入頁精簡版） |
| 上傳時間 | 2026-08-21 20:46 | 2026-08-23 17:32 |
| 到期 | **2026-11-19** | **2026-11-21**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | `86283a85-d0bb-40bc-ad0c-94e5feb8be94`（1728） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 1410 → 1728 |
...
- 目前先行測試的 build `202608231728`（2026-08-23，登入頁精簡版）：帶 KIOSK 兩參數、**不帶任何 E2E define**——帶入鈕與內嵌憑證皆為編譯期死碼（第 3 關印「測試期/kiosk dart-define：KIOSK_EMAIL KIOSK_PASSWORD」存證，清單裡沒有任何 E2E 項；純乾淨版 `202608231321` 當時印的是「無（正式版行為）」）。§8.6 的「最終版要拆」自此兌現；兩個測試帳號（測試員/測試醫師）仍在
