...
>
> 最後更新：2026-08-22 晚間（先行測試最新 build `202608222012`；打包腳本已支援 E2E 環境變數）
...
`202608222012`（本地時間），若 `date -u` 算出來 ≤ 它，就改用 `date +%Y%m%d%H%M`（本地）
明確帶 `--build-number=` ——只要單調遞增即可，UTC/本地混用在數值上無害。
目前群組狀態：**先行測試群組最新 = `202608222012`；4 人全群組仍停在 `202608211213`**
（0710/1528/1753/1816/2012 都只發先行測試）。
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202608222012`（版本 1.0.0，最新） |
| 上傳時間 | 2026-08-21 20:46 | 2026-08-22 20:14 |
| 到期 | **2026-11-19** | **2026-11-20**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | `3e4cb901-4b01-4daa-9f75-ee5174c73e74` |
| 群組 build 歷史 | 只有 1213 | 1528 → 1753 → 1816 → 2012 |
...
- 目前先行測試的 build `202608222012`（2026-08-22 晚間重打；app 碼與 1816 相同——
  當晚的問診品質批次全在後端——重打是為了給測試者一個「最新版」錨點）：
  帶兩組帶入鈕、**不帶**自動登入。
