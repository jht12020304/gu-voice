...
>
> 最後更新：2026-08-23（先行測試最新 build `202608231850`＝場次日曆檢視版）
...
目前群組狀態：**先行測試群組最新 = `202608231850`（4 位測試員都在這個群組）；
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202608231850`（版本 1.0.0，最新＝場次日曆檢視版） |
| 上傳時間 | 2026-08-21 20:46 | 2026-08-23 18:56 |
| 到期 | **2026-11-19** | **2026-11-21**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | `b4714a78-6b13-4c2f-ac01-9eee137ddd47`（1850） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 1728 → 1805 → 1850 |
...
- 目前先行測試的 build `202608231850`（2026-08-23，場次日曆檢視版）：帶 KIOSK 兩參數、**不帶任何 E2E define**——帶入鈕與內嵌憑證皆為編譯期死碼（第 3 關印「測試期/kiosk dart-define：KIOSK_EMAIL KIOSK_PASSWORD」存證，清單裡沒有任何 E2E 項；純乾淨版 `202608231321` 當時印的是「無（正式版行為）」）。§8.6 的「最終版要拆」自此兌現；兩個測試帳號（測試員/測試醫師）仍在
