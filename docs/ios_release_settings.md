...
>
> 最後更新：2026-08-22 深夜（先行測試最新 build `202608222205`＝含問診流程病患端統一；打包腳本已支援 E2E 環境變數）
...
`202608222205`（本地時間），若 `date -u` 算出來 ≤ 它，就改用 `date +%Y%m%d%H%M`（本地）
明確帶 `--build-number=` ——只要單調遞增即可，UTC/本地混用在數值上無害。
目前群組狀態：**先行測試群組最新 = `202608222205`；4 人全群組仍停在 `202608211213`**
（0710/1528/1753/1816/2012/2041/2129/2205 都只發先行測試）。
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202608222205`（版本 1.0.0，最新＝含問診流程病患端統一） |
| 上傳時間 | 2026-08-21 20:46 | 2026-08-22 22:07 |
| 到期 | **2026-11-19** | **2026-11-20**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | 見 ASC（2205） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 2129 → 2205 |
...
- 目前先行測試的 build `202608222205`（2026-08-22 深夜；含全頁面視覺統一 PR #71/#72、
  稽核日誌 404/403 修復、問診流程病患端統一 PR #74）：帶兩組帶入鈕、**不帶**自動登入。
