...
>
> 最後更新：2026-08-23 凌晨（先行測試最新 build `202608230157`＝紅旗 toast/確認機制修復版）
...
`202608230157`（本地時間），若 `date -u` 算出來 ≤ 它，就改用 `date +%Y%m%d%H%M`（本地）
明確帶 `--build-number=` ——只要單調遞增即可，UTC/本地混用在數值上無害。
目前群組狀態：**先行測試群組最新 = `202608230157`；4 人全群組仍停在 `202608211213`**
（0710 起的每一顆都只發先行測試）。
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202608230157`（版本 1.0.0，最新＝紅旗 toast/確認機制修復版） |
| 上傳時間 | 2026-08-21 20:46 | 2026-08-23 02:00 |
| 到期 | **2026-11-19** | **2026-11-20**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | 見 ASC（0157） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 0136 → 0157 |
...
- 目前先行測試的 build `202608230157`（2026-08-23 凌晨；含紅旗 toast 誤鳴修復＋
  一鍵全部確認 PR #80，後端 bulk 端點已部署且已清空 127 筆測試遺留警示）：
  帶兩組帶入鈕、**不帶**自動登入。
