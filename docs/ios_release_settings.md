...
>
> 最後更新：2026-09-03（先行測試最新 build `202609030302`：iOS 前景橫幅＋通知中心保留修正版）
...
目前群組狀態：**先行測試群組最新 = `202609030649`（4 位測試員都在這個群組）；
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202609030302`（版本 1.0.0，最新＝iOS 前景橫幅＋通知中心保留修正版） |
| 上傳時間 | 2026-08-21 20:46 | 2026-09-03 11:10（Asia/Taipei） |
| 到期 | **2026-11-19** | **2026-12-02**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | `013243ea-3fb8-4e78-bd07-4b4e16e566c1`（0302） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 202608251014 → 202609021536 → 202609030302 |
...
- **KIOSK 模式（2026-08-23 起，產品功能非測試輔助）**：`KIOSK_EMAIL` /
  ...
  `202609030649` 打包時已輪替密碼，明文只存在該次打包程序的環境變數，程序結束後不留本機檔案；
  需要重打包時再輪替一次即可。
