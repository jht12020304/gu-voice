...
>
> 最後更新：2026-09-09（先行測試最新 build `202609090231`：推播 time-sensitive entitlement）
>
> ⚠️ **build 號的權威是 App Store Connect，不是這份文件。** 2026-09-03～09-09 之間本檔
> 自相矛盾過：這一行與下方 §7 表格寫 `202609030302`，而 §4 與其他段落寫 `202609030649`，
> 兩邊都沒有錯字，是 0649 上傳後只改了一半。打包前一律先問 ASC：
>
> ```bash
> cd backend && ./venv/bin/python - <<'PY'
> import sys; sys.path.insert(0, "/Users/chun/.appstoreconnect")
> import asc_api as a
> st, d = a.call("GET", "/v1/builds", {"filter[app]": a.APP_ID,
>     "fields[builds]": "version,processingState,uploadedDate",
>     "sort": "-uploadedDate", "limit": "5"})
> for b in d["data"]: print(b["attributes"])
> PY
> ```
>
> build number 只能遞增，挑錯會被 Apple 退件。
...
| 主要語言 | 繁體中文 | ✅ 可改 |
| 裝置上顯示名稱 | **`UroSense`**（`Info.plist` → `CFBundleDisplayName`） | ✅ 可改 |

> ✅ App Store Connect 與裝置主畫面的顯示名稱已統一為 `UroSense`；bundle ID、SKU 與
> Firebase App 身分仍維持原值。
...
`2026` < 現行 12 位數格式，CFBundleVersion 是逐段比較的，換過去會被判成未遞增而永久拒收。
⚠️ **UTC 時間可能比上一顆的號碼小**（歷史上有 build 用了本地時間打號）：上一顆是
`202608231410`（本地時間），若 `date -u` 算出來 ≤ 它，就改用 `date +%Y%m%d%H%M`（本地）
明確帶 `--build-number=` ——只要單調遞增即可，UTC/本地混用在數值上無害。
目前群組狀態：**先行測試群組最新 = `202609090231`（4 位測試員都在這個群組）；
已停用的 4 人舊群組仍停在 `202608211213`**
（0710 起的每一顆都只發先行測試）。
...
兩個群組、兩顆現行 build（先行測試拿最新、4 人全群組刻意停在舊版）：

| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202609090231`（版本 1.0.0，最新＝推播 time-sensitive entitlement，含快速開單頁） |
| 上傳時間 | 2026-08-21 20:47 | 2026-09-09 10:38（Asia/Taipei） |
| 到期 | **2026-11-19** | **2026-12-08**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | `74e1ee94-31d0-421e-af77-932372dcd48f`（202609090231） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 202609030302 → 202609030649 → 202609081800 → 202609090231 |
...
| 項目 | 值 |
|---|---|
| 狀態 | 兩顆 build 都「**準備測試**」，標記為「**內部**」 |
| 4 人全群組 | ✅ **內部測試（僅限已授權接觸病歷者）**，group id `17215977-9c5d-472d-9414-4798d3b6e11e` |
| 群組自動分發 | ❌ **兩個群組都刻意關閉，且此設定永久不可更改**——每顆新 build 都要手動掛上群組，是刻意保留的人為檢查點 |
| 4 人群組測試員 | ✅ 2 位已在群組（狀態「已邀請」，2026-08-21） |
...
>
> ⚠️ **2026-08-21 決策變更：使用者拍板加入 4 位測試員。**
> 原本的「第一版只裝自己一台」已被推翻。使用者的理由是**帳號發放由他自己控制**——
> 即 App 一打開就是登入頁，沒有醫師帳號就什麼都看不到，PHI 暴露發生在「發出真實醫師帳號」
> 那一刻而不是「加進 TestFlight」那一刻。2026-09-02 已再補臨床帳號 row-level scope；
> 但帳號授權仍是必要控制點，未授權測試者的 staging 尚未建立。
>
> 🛑 **現行風險：**內測包仍打**生產後端**（沒有 staging）。doctor 與臨床 admin
> 只讀自己被指派的病患，system admin 仍有全院稽核視野；推播內容仍可能在鎖定畫面顯示 PHI。
...
## 8.6 測試期登入輔助（2026-08-22 晚間版；最終版要拆）

三個機制，全部由 build 時的 `--dart-define` 控制，**正式版不帶參數＝整段在編譯期
就是死碼**（「登入最後才設定」的機制，不用改碼）：

| define | 控制什麼 |
|---|---|
| `E2E_EMAIL` / `E2E_PASSWORD` | 登入頁「帶入病患帳號」鈕 |
| `E2E_DOCTOR_EMAIL` / `E2E_DOCTOR_PASSWORD` | 登入頁「帶入醫師帳號」鈕 |
...
- **KIOSK 模式（2026-08-23 起，產品功能非測試輔助）**：`KIOSK_EMAIL` /
  ...
  無真實資料的 patient 帳號。帳號為「候診 Kiosk（勿用真實資料）」，識別資料不抄進公開文件。
  **每次重打包都輪替一次密碼**（`202609030649`、`202609081800` 都做過），明文只存在該次
  打包程序的環境變數，程序結束後不留本機檔案。⚠️ 因此輪替之後、新 build 裝上去之前，
  現場 iPad 上舊 build 的 kiosk 自動登入會失敗——重打包前先確認沒有人正在用 kiosk。
- 目前先行測試的 build `202609030649`（2026-09-03）：帶 KIOSK 兩參數、**不帶任何 E2E define**——登入頁直接顯示「開始語音問診」，按下後以候診專用 patient 帳號自動登入並進入選症狀頁，使用者不必輸入帳密。第 3 關只印 define 名稱、不印值；產物內已確認 KIOSK 自動登入資料存在。兩個測試帳號（測試員/測試醫師）仍在
  production 供 e2e 工具使用，**生產收真病歷前仍須停用測試醫師帳號**（撤銷條件不變）。
- **三位臨床測試帳號維持 `admin` 角色**（使用者指示：需要刪除問診權限），並以
  `license_number` 識別為「臨床 admin」。自 `202609021536` 起，臨床 admin 與 doctor
  都只看自己被指派的病患、場次、報告、紅旗與儀表板資料，也只可軟刪自己負責的場次；
  沒有執照欄位的 system admin 才保留全院稽核視野。
- 真實臨床帳號**四個**（2026-08-23 建三個、**2026-09-09 補上專案負責人本人那個**，
  皆由使用者指示；密碼原要求 0000，後端政策 ≥8 碼＋大小寫＋數字 → 統一 `Gu000000`；
  license_number 先以「待補」占位，請在 admin → 使用者管理補真值）：
  **名單與 email 見 ASC／使用者管理，不抄進公開 repo。**
  楊佳倫無「醫學生」角色可選 → 建為醫師、科別「泌尿科（醫學生）」。

  四個都是 `admin` ＋ 有 license_number ＝ **臨床 admin**：可被選為問診醫師，但只讀得到
  自己被指派的病患、也只能軟刪自己負責的場次（`get_clinician_scope_id()`）。
  自 2026-09-09 起 TestFlight 測試員與可選醫師是 1:1 的四個人。

  ⚠️ **四個讀得到真實病歷的臨床帳號共用同一組弱密碼 `Gu000000`。** 這在只有測試資料時
  可以接受，**生產開始收真病歷之前必須逐一改掉**——與下方「停用測試醫師帳號」是同一個
  前置條件，見 [`TODO.md`](TODO.md) §V8。帳號愈多，這條愈不能拖。
- E2E define 的傳入方式（2026-08-22 起）：`tool/build_ios_testflight.sh` 會把
  環境變數 `E2E_EMAIL`／`E2E_PASSWORD`／`E2E_DOCTOR_EMAIL`／`E2E_DOCTOR_PASSWORD`／
  `E2E_AUTO_LOGIN` 自動轉成 `--dart-define`（未設＝不帶＝正式版行為），
  不再需要手打 build 指令；第 3 關會印出「帶了哪幾個 define」（不印值）。
- 兩個專用測試帳號（production，皆無真實資料；密碼只在本機 /tmp，重開機即失）：
  - 病患：`hsuchun0304+guvoice-autotest@gmail.com`「測試員（勿用真實資料）」
  - 醫師：`hsuchun0304+guvoice-testdoctor@gmail.com`「測試醫師（勿用真實資料）」
- ⚠️ **醫師那組是「禁止內嵌 doctor 憑證」鐵律的使用者明示破例**（2026-08-22），
  僅限 TestFlight 內測建置；`202609021536` 起臨床帳號已依 `sessions.doctor_id` 隔離，
  不再因登入就讀到全院病歷。
  **生產開始有真病歷之前，先到 admin → 使用者管理停用這個測試醫師帳號**，
  之後的內測 build 不再帶 `E2E_DOCTOR_*`。web 公開版建置腳本照舊拒絕任何 E2E 憑證。
- 登入頁本身已依 taste-skill 重設計（PR #65；skill 裝在
  `.claude/skills/design-taste-frontend`，之後改前端視覺先載它）。
...
- ⬜ 在 iPhone（iOS 16+）上實際安裝並確認**打得開、功能能用**
  ...
  請測試者遇到問題時直接截圖
- 🟡 推播再次實測（2026-09-03）：醫師指派、worker、正確 iOS FCM token、production
  entitlement 與 iPhone 通知設定皆已驗證，但 Firebase 接受訊息後手機仍未顯示。
  build `202609030649` 新增 Apple 原生 APNs token 登記與後端直接 APNs 投遞（FCM 備援），
  Apple 已標為 VALID 並掛入 4 人先行測試群組；待楊佳倫手機更新並登入後複驗。
- ✅ 新問診通知改為指定醫師：病患資料頁必須先從動態帳號清單選醫師；已指派場次的
  `session_complete`、紅旗、`report_ready`／`report_failed` 與 dashboard 即時事件只送該醫師。
- ✅ 統一 App name 與裝置顯示名稱（2026-08-25：`UroSense`）
- 🛑 測試員仍須是獲授權院內人員：臨床 scope 已降低跨醫師暴露，但推播與報告仍含
  被指派病患的 PHI，system admin 也仍有全院稽核視野；未授權測試者應使用獨立 staging。
