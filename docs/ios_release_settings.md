...
>
> 最後更新：2026-08-23 凌晨（先行測試最新 build `202608230136`＝資料鏈路稽核修復版）
...
`202608230136`（本地時間），若 `date -u` 算出來 ≤ 它，就改用 `date +%Y%m%d%H%M`（本地）
明確帶 `--build-number=` ——只要單調遞增即可，UTC/本地混用在數值上無害。
目前群組狀態：**先行測試群組最新 = `202608230136`；4 人全群組仍停在 `202608211213`**
（0710 起的每一顆都只發先行測試）。
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202608230136`（版本 1.0.0，最新＝資料鏈路稽核修復版） |
| 上傳時間 | 2026-08-21 20:46 | 2026-08-23 01:40 |
| 到期 | **2026-11-19** | **2026-11-20**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | 見 ASC（0136） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 2246 → 0136 |
...
- 目前先行測試的 build `202608230136`（2026-08-23 凌晨；含資料鏈路稽核 12 項修復
  PR #78——SOAP 頁補齊、指導橫幅、警示篩選等，後端同步部署）：
  帶兩組帶入鈕、**不帶**自動登入。
