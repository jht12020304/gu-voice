...
>
> 最後更新：2026-08-23（先行測試最新 build `202608231341`＝kiosk 進場版：登入頁「開始語音問診」大按鈕）
...
`202608231341`（本地時間），若 `date -u` 算出來 ≤ 它，就改用 `date +%Y%m%d%H%M`（本地）
明確帶 `--build-number=` ——只要單調遞增即可，UTC/本地混用在數值上無害。
目前群組狀態：**先行測試群組最新 = `202608231341`；4 人全群組仍停在 `202608211213`**
（0710 起的每一顆都只發先行測試）。
...
| 項目 | 4 人全群組 | 先行測試群組 |
|---|---|---|
| Build | `202608211213`（版本 1.0.0） | `202608231341`（版本 1.0.0，最新＝kiosk 進場版） |
| 上傳時間 | 2026-08-21 20:46 | 2026-08-23 13:53 |
| 到期 | **2026-11-19** | **2026-11-20**（+90 天不可延長）⚠️ 排行事曆 |
| Delivery UUID | `b37056bb-0b50-453c-a015-b0e390d3bdbd` | 見 ASC（1341） |
| 群組 build 歷史 | 只有 1213 | 1528 → … → 1321 → 1341 |
...
- 目前先行測試的 build `202608231341`（2026-08-23）：帶 KIOSK 兩參數、**不帶任何 E2E define**——帶入鈕與內嵌憑證皆為編譯期死碼（第 3 關輸出「測試期 dart-define：無（正式版行為）」
  存證）。§8.6 的「最終版要拆」自此兌現；兩個測試帳號（測試員/測試醫師）仍在
