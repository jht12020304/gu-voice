...
`2026` < `202608221816（先行測試群組，雙帶入鈕版）；全群組仍在 202608211213（0710/1528/1753/1816 都只發先行測試）`，CFBundleVersion 是逐段比較的，換過去會被判成未遞增而永久拒收。
...
- 目前先行測試的 build `202608221816`：帶兩組帶入鈕、**不帶**自動登入。
