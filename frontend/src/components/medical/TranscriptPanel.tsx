...
type TranscriptRole = 'patient' | 'assistant' | 'system';
...
const roleStyle: Record<TranscriptRole, RoleStyle> = {
  patient:   { color: 'text-primary-700 dark:text-primary-300', bg: 'bg-primary-50 border-primary-100 dark:bg-primary-950/40 dark:border-primary-800', align: 'right' },
  assistant: { color: 'text-amber-700 dark:text-amber-300', bg: 'bg-amber-50/60 border-amber-100 dark:bg-amber-950/30 dark:border-amber-800', align: 'left' },
  system:    { color: 'text-ink-muted', bg: 'bg-surface-tertiary border-edge dark:bg-dark-border/50 dark:border-dark-border', align: 'center' },
};
...
export default function TranscriptPanel({
    ...
    return (
      <div className={`card overflow-hidden p-0 ${className}`}>
        <div className="px-5 py-4 flex items-center gap-3">
          ...
          <span className="text-body text-ink-muted">{t('transcript.loading')}</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`card overflow-hidden p-0 ${className}`}>
        <div className="border-b border-edge px-5 py-4 dark:border-dark-border">
          <h3 className="text-h3 font-semibold text-ink-heading dark:text-white">{t('transcript.title')}</h3>
          <p className="mt-1 text-small text-ink-muted">{t('transcript.emptyTitle')}</p>
        </div>
      </div>
    );
  }
  ...
  const patientCount = sorted.filter((c) => c.role === 'patient').length;
  const assistantCount = sorted.filter((c) => c.role === 'assistant').length;

  const filterOptions: Array<{ value: 'all' | 'patient' | 'assistant'; label: string }> = [
    { value: 'all', label: t('transcript.filter.all') },
    { value: 'patient', label: t('transcript.filter.patient') },
    { value: 'assistant', label: t('transcript.filter.assistant') },
  ];
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-tiny text-ink-muted">{t('transcript.messageCount', { count: sorted.length })}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="rounded-pill bg-surface-tertiary px-2.5 py-0.5 text-tiny text-ink-muted dark:bg-dark-border">
            {t('transcript.summary', { patientCount, assistantCount })}
          </span>
          {collapsible ? (
            <svg
        ...
        <div className="border-t border-edge px-5 py-4 dark:border-dark-border">
          <div className="space-y-3">
            ...
            {hasMore && (
              ...
              >
                {t('transcript.viewFull', { count: sorted.length - 4 })}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="border-t border-edge dark:border-dark-border">
          <div className="flex flex-col gap-3 border-b border-edge px-5 py-4 dark:border-dark-border">
            ...
            <p className="text-small text-ink-muted">
              {t('transcript.visibleCount', { visible: visibleMessages.length, total: sorted.length })}
            </p>
          </div>
          <div className={`${maxHeightClass} overflow-y-auto px-5 py-4`}>
            {visibleMessages.length > 0 ? (
              ...
              <p className="py-6 text-center text-body text-ink-muted">{t('transcript.noMatches')}</p>
            )}
          </div>
        </div>
      )}
...
function MessageBubble({ message, compact = false }: { message: Conversation; compact?: boolean }) {
  const { t } = useTranslation('dashboard');
  const role: TranscriptRole = (['patient', 'assistant', 'system'] as const).includes(
    message.role as TranscriptRole,
  )
    ? (message.role as TranscriptRole)
    : 'system';
    ...
    <div className={`flex gap-3 ${isPatient ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`max-w-[75%] ${isPatient ? 'text-right' : ''}`}>
        <div className="flex items-center gap-2 mb-0.5">
          {!isPatient && <span className={`text-tiny font-medium ${cfg.color}`}>{roleLabel}</span>}
        ...
        {message.sttConfidence !== undefined && message.sttConfidence < 0.8 && (
          <p className="mt-0.5 text-tiny text-ink-placeholder">
            {t('transcript.sttConfidence', { value: Math.round(message.sttConfidence * 100) })}
          </p>
        )}
      </div>
    </div>
