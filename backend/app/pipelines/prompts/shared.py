...
URO_RED_FLAGS: list[dict[str, Any]] = [
    {
        ...
        "triggers_by_lang": {
            ...
            "ja-JP": [
                ...
                "排尿できない",
                "全く排尿できない",
                # 2026-08-20 稽核 RF-4：共現組的裸「できない／できません」被移除
                #（臨床反義：「我慢できない」＝尿意切迫，被判成尿滯留），
                # 那條詞承載的**真**尿閉講法改成從「排尿／おしっこ／小便」開頭的
                # 相鄰片語收錄——語意明確、不可能反義。
                "排尿できません",
                "排尿ができない",
                "排尿ができません",
        ...
        "trigger_cooccurrence": [
            {
                ...
                "site_terms": [
                    ...
                    "尿", "小便", "小號", "排尿", "膀胱", "解手",
                    # ja-JP
                    # ⚠️ 2026-08-20 稽核 RF-4：「下腹」を**削除**した。
                    # 理由：本表は**全言語の和集合**（不變式 #25）で、中国語の
                    # 「下腹」＝下腹部一般（生理痛・胃腸の張り）にそのまま当たる。
                    #   「我這兩天下腹脹得很難受，應該是吃壞肚子」→ 下腹×脹得 → critical
                    #   「生理期下腹很脹很痛」                    → 下腹×很脹 → critical
                    # ＝ 消化器/婦人科の訴えで問診を中止していた。
                    # 漏報しない理由（#22 の挙証）：日本語の尿閉の訴えは必ず
                    # 尿語（尿／おしっこ／小水／尿意）か「膀胱」を伴う。実測でも
                    #   「昨日の夜から全然おしっこが出なくて下腹が張って痛いです」→ おしっこ×出なく
                    #   「熱は普通ですが、尿が全く出なくて下腹がパンパンです」  → 尿×全く出
                    #   「朝から一滴も排尿できていません、下腹部が苦しいです」  → 尿×一滴も
                    # と、既存の MUST_FIRE 語料は全て別の足場で命中する。
                    # 「下腹だけ」で尿閉を訴える形（尿語ゼロ）は構造上
                    # 胃腸の張りと区別できない ＝ 規則層で拾うべきではない（語意層が後備）。
                    "おしっこ", "小水", "尿意",
                    # ko-KR
                    "소변", "오줌", "배뇨", "방광",
                ...
                "acuity_terms": [
                    ...
                    # ⚠️ 2026-08-20 稽核 RF-4：裸「できない／できません／できていません」を
                    # **削除**。臨床的に**反義**の語に当たっていた：
                    #   「トイレを我慢できないくらい尿意が強いです」＝尿意切迫（蓄尿症状）
                    #   → 尿×できない → 急性尿閉(critical) で問診中止（実測）。
                    # しかも「我慢できない」は detector の `_POST_CUE_FALSE_FRIENDS`
                    # （＝否定守衛が「症状の強調表現」として**放行**する語）でもあるので、
                    # 否定守衛では絶対に止まらない。短い語幹の和集合誤配（#25）の典型。
                    # 漏報しない理由（#22 の挙証）：本当の尿閉の「できない」形は必ず
                    # 排尿語が直前に来る（排尿できない／おしっこができない）ので、
                    # それらを triggers 側に相隣句として収録した（上記 ja-JP triggers）。
                    # 副詞挿入形（全く／全然／一滴も＋出ません）は上の行が拾う。
                    "張って", "張った",
                    "パンパン", "苦しい", "溜まって",
                    # ko-KR
    ...
    },
    {
        ...
        # ⚠️ 2026-08-20 稽核 RF-3（臨床已拍板）：裸的**血塊詞**從 triggers 移除。
        # 缺陷：本紅旗的臨床定義是「**尿裡**的大量血/血塊」，但裸「血塊／血の塊／혈전」
        # 不要求任何泌尿軸，於是與泌尿完全無關的血栓/瘀血描述被判 critical、中止問診：
        #   「我腳上有一塊血塊瘀青」        → gross_hematuria_heavy(critical)
        #   「足に血の塊ができました」      → 同上
        #   「다리에 혈전이 생겼대요」（＝下肢深部靜脈血栓，**別的**急症）→ 同上
        # 為什麼不會造成漏報（#22 舉證，逐字面）：這三個字面**原封不動**留在
        # `trigger_cooccurrence.urine_x_heavy_blood` 的 acuity_terms 裡，只是多要求
        # 同一/相鄰子句裡有尿液詞（尿／小便／馬桶／おしっこ／トイレ／소변／urine／pee／
        # tiểu…，全語言和集合）。實測「尿裡有血塊」「おしっこに血の塊が混じっています」
        # 「소변에 혈전이 섞여 나와요」三語都仍然命中 critical。
        # 真正的大量血尿病患不可能不提到尿——不提尿的「血塊」根本不是本紅旗。
        # 其餘 triggers（大量血尿／血尿很多／整個都是血／一大堆血）保留：前兩者自帶
        # 「血尿」＝已含泌尿軸；後兩者是本輪未經臨床覆核的字面，依 #22 保留並記在
        # tests/unit/pipelines/test_red_flag_audit_2026_08.py 的 KEPT_LITERALS。
        "triggers": [
            "大量血尿",
            "整個都是血",
            "血尿很多",
        ...
        "triggers_by_lang": {
            ...
            "ja-JP": [
                "大量の血尿",
                # 「血の塊」は RF-3 で削除（共現組 acuity に温存、上の註記参照）
                "尿が真っ赤",
                "血だらけの尿",
            ],
            "ko-KR": [
                "다량의 혈뇨",
                # 「혈전」は RF-3 で削除（＝血栓。下肢 DVT を critical 血尿と誤判）
                "피가 섞인 소변이 많아요",
                "새빨간 소변",
            ],
            "vi-VN": [
        ...
        "trigger_cooccurrence": [
            {
                ...
                "acuity_terms": [
                    ...
                    # ⚠️ 2026-08-20 稽核 RF-4：裸「塊」を**削除**（「かたまり」は残す）。
                    # 理由：1 文字の CJK 字面は和集合比對（#25）で最も危険。
                    #   「尿路結石の塊が出ました」        → 尿×塊 → critical（結石＝別疾患）
                    #   「我尿完之後石頭一塊一塊排出來」  → 尿×塊 → critical（中国語の量詞「一塊」）
                    # 漏報しない理由（#22 の挙証）：血塊の訴えは「血の塊／血のかたまり／
                    # 血塊／血凝塊／凝血塊／血が混じったかたまり」のいずれかで、
                    # そのすべてが本表に個別収録済み（「かたまり」単独も残してある）。
                    # 実測「おしっこに血の塊が混じっています」「尿裡有血塊」は命中維持。
                    # 「かたまり」を残す理由：3 モーラの仮名列で、中国語・韓国語の
                    # 文には出現しない＝和集合の巻き添えが構造上起きない。
                    "血だらけ", "血の塊", "血のかたまり", "かたまり",
    ...
    },
    {
        ...
        # ⚠️ 2026-08-20 稽核 RF-3（臨床已拍板）：**不要求泌尿軸的裸發燒詞**移除。
        # 缺陷：本紅旗的臨床定義是「尿路感染 ＋ 全身性感染徵象」＝兩個軸，但
        # 「高燒／高熱／고열／high fever／sốt cao」單獨一個詞就判 critical、中止問診：
        #   「我上個月因為流感發高燒」            → urosepsis(critical)
        #   「i had a high fever last week from the flu」→ 同上
        #   「tuần trước tôi bị sốt cao vì cúm」  → 同上
        # 流感／一般感冒的發燒被當成尿路敗血症，是最常見的一類誤中止。
        # 為什麼不會造成漏報（#22 舉證，逐字面）：五個字面全部**原封不動**留在
        # `trigger_cooccurrence.urinary_x_systemic_infection` 的 acuity_terms
        #（「高燒」「高熱」「고열」，英文由前緣詞邊界的「fever」涵蓋 high fever，
        #  越南文由「sốt」涵蓋 sốt cao），只是多要求同一或**相鄰**子句
        #（本組 `cross_clause: True`）有泌尿詞。實測「我發高燒而且小便會痛」
        # 五語都仍然命中 critical。
        # 「意識不清」同理移除：它也在 acuity_terms 裡，且本紅旗的臨床實體是
        # urosepsis——沒有任何泌尿描述的意識改變不是本紅旗（語意層仍獨立判）。
        # ⚠️ 保留的字面與理由見 test_red_flag_audit_2026_08.py 的 KEPT_LITERALS：
        #   「寒顫／chills／悪寒／오한／ớn lạnh」同屬「單軸即 critical」這一類，
        #   但**不在本輪臨床拍板的清單內**，依 #22（抑制/收窄的舉證責任在提出方）
        #   不擅自收窄；其餘四語的意識改變詞（altered consciousness／意識がもうろう／
        #   의식이 흐려짐／rối loạn ý thức）則是共現組**接不住**的字面（不在 acuity_terms），
        #   移除會造成真漏報，故保留。
        "triggers": [
            "寒顫",
            "發燒加排尿痛",
        ...
        "trigger_cooccurrence": [
            {
                ...
                "acuity_terms": [
                    ...
                    "發燒", "高燒", "燒到", "燒起來", "燒了", "在燒",
                    "發熱", "度的燒",
                    # ⚠️ 2026-08-20 稽核 RF-4：裸「體溫／体温／temperature」を削除。
                    # 三者とも**度量名詞**であって発熱の訴えではない：
                    #   「護理師剛剛幫我量體溫，我主要是想講排尿的問題」→ 尿×體溫 → critical
                    #   「毎朝体温を記録しています、おしっこの回数が多いだけです」→ 同上
                    #   「my temperature was normal, i just have some urinary frequency」→ 同上
                    #   （「体温は平熱でした」まで critical になっていた ＝ 正常と言っているのに）
                    # 漏報しない理由（#22 の挙証）：この語が**唯一の足場**だった真人語序は
                    # 「体温を測ったら三十八度五分」「我量起來體溫三十八度九」のように
                    # **必ず発熱の実測値を伴う**。そこで度量名詞ではなく
                    # 「発熱域の度数」そのものを収録した（下の「三十八度／三十九度／四十度」）。
                    # 平熱（三十六度台・三十七度台）は意図的に収録しない＝
                    # 「體溫三十六度五」は命中しない。英語は "running a temperature"
                    # という**発熱を意味する慣用句**の形で収録し直した（下の en-US 段）。
                    "體溫高", "體溫升高", "體溫偏高", "體溫飆",
                    # 発熱域の度数（CJK 三言語共通の字面。和集合なので一箇所で足りる）
                    "三十八度", "三十九度", "四十度", "38度", "39度", "40度",
                    "38도", "39도", "40도", "삼십팔도", "삼십구도",
                    "身體很熱", "身體熱", "全身很熱", "全身熱",
                    "渾身很熱", "渾身熱",
                    "身體發燙", "全身發燙", "額頭發燙",
                    ...
                    "度の熱",
                    # ⚠️ 2026-08-20 稽核 RF-4：裸「震え／ふるえ」を削除（「体温」は上記）。
                    # 振戦は発熱以外の原因（緊張・薬剤性・本態性・カフェイン）が大半で、
                    # 全身性感染徴候とは言えない：
                    #   「緊張して手が震えます、おしっこの回数だけ気になります」→ critical（実測）
                    # 漏報しない理由（#22 の挙証）：悪寒戦慄の訴えは日本語では必ず
                    # 「悪寒」「寒気」を伴う（既収録）。既存 MUST_FIRE の
                    # 「寒気がして体が震えるしおしっこも痛いです」「発熱と震えがあって…」
                    # は二つとも悪寒/発熱側で命中を維持（実測）。
                    # 悪寒を伴う戦慄のオノマトペ「ぞくぞく／がたがた震え」を補って
                    # 発熱側の取りこぼしを埋める。
                    "悪寒", "寒気", "ぞくぞく", "がたがた震え", "寒くて震え",
                    ...
                    # ⚠️ 2026-08-20 稽核 RF-4：裸「떨리」を削除。
                    # 「손이 떨리다」＝手の震え（カフェイン・不安・薬剤性）で、
                    # 全身性感染徴候ではない：
                    #   「커피 때문에 손이 떨리는데 소변 검사 받으러 왔어요」→ critical（実測）
                    # 漏報しない理由（#22 の挙証）：韓国語の悪寒戦慄は「오한」「한기」
                    #「덜덜 떨다」「오슬오슬」で表現され、いずれも収録済み。
                    # 既存 MUST_FIRE「몸이 계속 떨리고 오한이 나면서 소변이 탁해요」は
                    # 「오한」側で命中を維持（実測）。
                    "오한", "한기", "오슬오슬", "덜덜", "몸살",
                    ...
                    # ⚠️ 2026-08-20 稽核 RF-4：裸 "shaking" と裸 "temperature" を削除。
                    #   "my hands keep shaking from the medication and i pee often"
                    #     → pee × shaking → urosepsis(critical)（実測。振戦は感染徴候ではない）
                    #   "my temperature was normal, i just have some urinary frequency"
                    #     → urinary × temperature → critical（**正常だと言っている**のに）
                    #   "the nurse took my temperature at the desk, i am here about my urine flow"
                    #     → 同上（temperature は度量名詞であって発熱の訴えではない）
                    # 漏報しない理由（#22 の挙証）：英語の発熱の訴えは "fever/febrile"
                    #（前緣詞邊界なので fevers/feverish も拾う）か、慣用句
                    # "running a temperature"（＝発熱している）で、後者を句として収録し直した。
                    # 既存 MUST_FIRE の 2 句 "running a temperature …" は命中を維持（実測）。
                    # 悪寒戦慄側は "chill/chills/shiver/shivering/rigor" が既にあり、
                    # 発熱を伴う戦慄の言い方 "shaking chills" を句として補った。
                    "fever", "febrile", "chill", "shiver", "rigor",
                    "shaking chills", "shaking with chills",
                    "running a temperature", "high temperature", "temperature of",
                    "hot and cold", "burning up", "sepsis",
                    "septic", "confused and",
                    # vi-VN
...
def normalize_canonical_id(value: Any) -> str:
    """把「紅旗身份字串」正規化成穩定的比較鍵（2026-08-20 稽核 D-7）。

    為什麼需要（缺陷）：語意層對**不在內建目錄裡**的紅旗（LLM 自創命名）是拿
    `raw_title` 當 canonical_id 用的，而 LLM 每一輪的大小寫／前後空白／內部空白
    都可能不同：

        turn 1 → "Testicular Torsion Suspected"
        turn 2 → "testicular torsion suspected"
        turn 3 → " Testicular  Torsion Suspected "

    三者是同一個紅旗，但字串不同 →
      - `_merge_and_deduplicate` 的 dedup key 不同 → 同一輪的兩層合不起來；
      - `alert_dedup.alert_dedup_identity` 的 Redis 欄位不同 → 跨輪去重整個失效
        → 同一個紅旗每輪重新寫一筆 alert、重新廣播一次（護理站警示疲勞、
          research analytics 的紅旗計數灌水）。

    正規化＝lowercase ＋ strip ＋ 內部連續空白摺疊成一個空白。
    對內建目錄的 snake_case canonical_id 是**恆等變換**（本來就全小寫無空白），
    所以既有行為不受影響。
    """
    if value is None:
        return ""
