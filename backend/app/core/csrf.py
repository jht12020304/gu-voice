...
body 路徑豁免（雙路徑 refresh）
------------------------------
double-submit 只適用「refresh token 由瀏覽器隨 cookie 自動附帶」的路徑。
/auth/refresh、/auth/logout 的 body 路徑（跨站部署後備：SameSite cookie 不隨
跨站 XHR 送出）中，token 由前端 JS 自 localStorage 讀出並顯式放入 JSON body——
跨站攻擊者無法讀取受害者的 localStorage、也無法令瀏覽器自動附帶它，與
Authorization: Bearer header 同級的自證憑證，無 CSRF 攻擊面，故該路徑跳過
double-submit 驗證。注意：refresh token 絕不可同時放回「可被瀏覽器自動附帶」
的儲存又走豁免路徑，否則豁免前提失效。
"""
